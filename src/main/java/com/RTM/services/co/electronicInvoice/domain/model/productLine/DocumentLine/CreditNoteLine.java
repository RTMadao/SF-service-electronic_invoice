...
public class CreditNoteLine implements DocumentLine{
    ...
    public CreditNoteLine(LineID ID, String note, Unit creditedQuantity, Money lineExtensionAmount, PricingReference pricingReference, List<AllowanceCharge> allowanceCharge, List<TaxTotal> taxTotals, Item item, Price price) {
        this.ID = ID;
        this.note = note;
    ...
    @Override
    @XmlElement(name = "CreditedQuantity", namespace = "urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents‐2")
    ...
    public void setCreditedQuantity(Unit creditedQuantity) {
        this.creditedQuantity = creditedQuantity;
    }
    ...
    public void setCurrencyCode(String currencyCode){
        ...
        if(pricingReference != null)this.pricingReference.getAlternativeConditionPrice().getPriceAmount().setCurrencyID(currencyCode);
        if(allowanceCharge != null)this.allowanceCharge.forEach(allowanceCharge -> allowanceCharge.setCurrencyCode(currencyCode));
        if(taxTotals != null)this.taxTotals.forEach(taxTotal -> taxTotal.setCurrencyCode(currencyCode));
